...
- **4.6 frontend wiring (DEFERRED).** Backend `/api/v1/ifc` endpoint now accepts optional `cost_metadata`, `thermal_metadata`, `furniture_counts` dicts and the writer emits matching `Pset_ADS_*` property sets when they're provided. The frontend IFC export call site still sends only `{building, floor_plans}`. To enable enriched BIM exports, compute `estimateCost()` + `estimateThermal()` + aggregate `placeFurnitureForPlan()` counts at the call site and include them in the request body. Non-blocking — psets simply don't appear in the downloaded `.ifc` until wired.
