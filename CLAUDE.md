...
- **Phase 3.7 status** — 3.7a (undo/redo) + 3.7b (localStorage persistence) both shipped. 3.7c (backend-backed persistence) still to do: needs SQL `FloorPlanRun` table, `user_overrides` field on the FloorPlan model, save/load endpoints, and cross-device sync. The localStorage approach already survives page reloads in the same browser — 3.7c is only required for multi-device editing.
- **Persistence — localStorage quota** — Each `EditedFloorPlanEntry` is a full FloorPlan JSON (rooms/walls/doors/windows with polygons) — typically 20–60 KB per floor per building. localStorage caps at ~5 MB per origin, so ~80–250 edited floor plans is a reasonable worst case. No pruning yet; if quota errors appear, add an LRU eviction to `editedFloorPlans` keyed by `savedAt`.
- **Persistence — no cross-device sync** — 3.7b is browser-local. Switching machines or clearing browser storage loses edits. 3.7c will upgrade to server-side.
